...
<head>
    ...
    <style>
        ...
        .amount-val { text-align: right; font-weight: bold; color: #e11d48; }
        .paid-to    { font-size: 11px; color: #4f46e5; font-weight: 500; }
        .ref-no     { font-size: 9px; color: #94a3b8; text-transform: uppercase; margin-top: 2px; }
...
<table>
    ...
    @foreach($expenses as $i => $expense)
        <tr>
            ...
            <td>
                <div class="paid-to">{{ $expense->paid_to ?? '—' }}</div>
                @if($expense->reference_no)
                    <div class="ref-no">{{ $expense->reference_no }}</div>
                @endif
            </td>
            <td class="amount-val">{{ number_format($expense->amount, 2) }}</td>
        </tr>
    @endforeach
    <tr class="total-row">
        <td colspan="6">Total Approved Expense</td>
