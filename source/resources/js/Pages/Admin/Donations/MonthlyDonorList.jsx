    ...
    <AdminLayout pageName="Monthly Donor Tracking">
      ...
      <form onSubmit={handleFilterSubmit} className="mb-6 bg-white p-4 rounded-sm border border-slate-200 shadow-sm">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
          ...
          <div className="flex gap-2">
            <button type="submit"
                    className="flex-1 bg-indigo-600 text-white text-xs font-bold py-2 rounded h-10 flex items-center justify-center gap-2 hover:bg-indigo-700 transition">
              <Filter size={14}/> Filter
            </button>
            <button type="button" onClick={handleClear}
                    className="bg-slate-100 text-slate-600 text-xs font-bold px-3 rounded h-10 flex items-center justify-center hover:bg-slate-200 transition">
              <RotateCcw size={14}/>
            </button>
          </div>
        </div>
      ...
      <div className="rounded-sm border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="max-w-full overflow-x-auto">
          <table className="w-full table-auto text-left min-w-[900px]">
            <thead>
            <tr
              className="bg-slate-50 border-b border-slate-200 text-slate-600 uppercase text-xs font-bold tracking-wider">
            ...
            {donors.data.map((donor) => (
              <tr key={donor.id} className="hover:bg-slate-50/50">
                ...
                <td className="px-6 py-4 text-center">
                  {donor.has_paid ? (
                    <span
                      className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-[10px] font-bold uppercase">Paid</span>
                  ) : (
                    <span
                      className="bg-rose-100 text-rose-700 px-3 py-1 rounded-full text-[10px] font-bold uppercase">Unpaid</span>
                  )}
                </td>
                <td className="px-6 py-4 text-right">
                  {!donor.has_paid && (
                    <button onClick={() => openPayModal(donor)}
                            className="bg-indigo-600 text-white px-4 py-2 rounded text-xs font-bold hover:bg-indigo-700">Pay
                      Now</button>
                  )}
                </td>
              </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div
            className="w-full max-w-md bg-white rounded-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 bg-slate-50">
              <h3 className="font-bold text-slate-800">New Donation Entry</h3>
              <button onClick={closeModal} className="text-slate-400 hover:text-rose-500 transition"><X size={20}/>
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={data.receipt_no} onChange={e => setData('receipt_no', e.target.value)}
                         placeholder="Enter receipt ID" className="w-full rounded-lg border-slate-200 text-sm"/>
                </div>
              </div>


              <button type="submit" disabled={processing}
                      className="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold hover:bg-indigo-700 shadow-lg flex items-center justify-center gap-2 transition">
